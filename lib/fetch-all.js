define(['exports', 'module', './parse-links'], function (exports, module, _parseLinks) {
   /**
    * Copyright 2015 aixigo AG
    * Released under the MIT license.
    * http://laxarjs.org/license
    */

   'use strict';

   module.exports = fetchAll;

   function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { 'default': obj }; }

   var _parseLinks2 = _interopRequireDefault(_parseLinks);

   /**
    * Fetch the given URL and keep following the "next" link
    * until all results are returned.
    * @param {String} url
    * @param {Object} options
    * @return {Promise}
    */

   function fetchAll(url, options) {
      return fetch(url, options).then(function handleResponse(response) {
         var promise = response.json();
         var links = response.headers.get('Link');
         var next = links && (0, _parseLinks2['default'])(links).next;

         if (next) {
            return fetch(next, options).then(handleResponse).then(function (tail) {
               return promise.then(function (head) {
                  return head.concat(tail);
               });
            });
         } else {
            return promise;
         }
      });
   }
});
//# sourceMappingURL=fetch-all.js.map
